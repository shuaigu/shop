...
//            $appoint_time = $appoint_time / 1000;//毫秒时间戳转为秒时间戳
            $appoint_day = date('Y-m-d',$appoint_time);
            $business_start_time = strtotime($appoint_day.' '.$this->shop['business_start_time']);
            $business_end_time   = strtotime($appoint_day.' '.$this->shop['business_end_time']);

            if ($appoint_time > 1 && $business_start_time > $business_end_time && ($business_start_time > $appoint_time && $business_end_time < $appoint_time)) {
            ...
            if('submit' === $this->action && empty($mobile) && $this->params['order_type'] == OrderEnum::DINING_TYPE_TAKE_EAT){
...
//        }
    ...
    public function getOrderAmount(){
        $order_amount = round($this->total_amount - $this->discount_amount,2);
        if($order_amount < 0){
            $order_amount = 0;
        }
        return $order_amount;
...
//        $order->pay_way         = 0;
        ...
        $order->shop_desk_id    = $this->params['desk_id'] ?? null;
