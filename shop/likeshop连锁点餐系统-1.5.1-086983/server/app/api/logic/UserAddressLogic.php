...
use app\common\{model\DevRegion, model\shop\Shop, server\AreaServer, model\user\UserAddress, server\ConfigServer};
use think\facade\Db;
use think\Exception;
...
class UserAddressLogic
{
     ...
     * @date 2021/9/7 16:27
     */
    public static function lists($get,int $user_id)
    {
        $longitude = $get['longitude'] ?? 0;
        $latitude = $get['latitude'] ?? 0;
        //配送距离
        $delivery_distance = Shop::where(['id'=>$get['shop_id'] ?? 0])->value('delivery_distance') ?? 0;
        //用st_distance_sphere函数计算两点记录，单位米，这里换成千米
        if (!$longitude || !$latitude) {
            $field = 'id,contact,telephone,province_id,city_id,district_id,location,address,is_default,longitude,latitude,0 as distance';
        } else {
            $field = 'id,contact,telephone,province_id,city_id,district_id,location,address,is_default,longitude,latitude,round(st_distance_sphere(point('.$longitude.','.$latitude.'),point(longitude, latitude))/1000,2) as distance';
        }
        ...
        foreach ($info as &$item) {
            ...
            //判断距离
            if ($delivery_distance >= $item['distance']) {
                $lists['normal'][] = $item;
            } else {
                $lists['abnormal'][] = $item;
            }
//            $item['distance'] = round($item['distance'],2).'km';
    ...
    public static function handleRegionField($keyword, $level = 1, $parent_id = '')
    {
        ...
        foreach ($lists as $k => $v) {
            ...
            if ($keyword == trim($v['name']) || strpos($keyword, $v['name']) !== false) {
                $data = $v['id'];
            }
        }
        return $data;
