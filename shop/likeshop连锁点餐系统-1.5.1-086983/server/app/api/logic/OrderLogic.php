...
//                    ->sendNotice()      //发送通知
    ...
    public static function getOrderDetail($id,$user_id)
    {
        $detail = Order::with(['order_goods','shop'])
                ...
                ->append(['order_type_text','order_status_text','pay_btn','cancel_btn','again_btn','delivery_address','desk_info'])
                ->where(['id'=>$id,'user_id'=>$user_id])
                ->find()
                ->toArray();
        $detail['order_cancel_time'] = ConfigServer::get('transaction', 'unpaid_order_cancel_time', 5) * 60;
