...
class LoginLogic extends Logic
{
    ...
    public static function register($post)
    {
        ...
        try {
            ...
            $nickname = '用户'.$sn;
            $now   = time(); //创建时间
            $salt = substr(md5($now . $sn), 0, 4);//随机4位密码盐

            $avatar = ConfigServer::get('website', 'user_image', 'static/common/image/default/user.png');
            ...
            User::field(['id', 'nickname', 'avatar', 'disable','is_new_user'])
                ->where(['id' => $user_id])
                ->find();
    ...
    public static function oaLogin($post)
    {
        //微信调用
        try {
            $user = self::getWechatOaResByCode($post);
            //添加或更新用户
            $user_id = UserAuth::alias('au')
                ->join('user u', 'au.user_id=u.id')
                ->where(['u.del' => 0])
            ...
            return $user_info->toArray();
        } catch (Exception|AuthorizeFailedException $e) {
            self::$error = $e->getMessage();
            return false;
        }
    }
    ...
    public static function updateUser($post,$user_id)
    {
        ...
        User::where(['id'=>$user_id])->update(['nickname'=>$post['nickname'],'avatar'=>UrlServer::setFileUrl($post['avatar']),'is_new_user'=>0]);
        return true;
    }
     ...
     * @date 2023/10/8 9:58
     */
    public static function resetPwd(array $post)
    {
        ...
        try {
            ...
            $user->password =   create_password($password,$user->salt);
            $user->save();
