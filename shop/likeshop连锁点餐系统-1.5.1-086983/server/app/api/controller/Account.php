...
use app\api\validate\ResetPwdValidate;
use app\api\validate\UpdateUserValidate;
use app\api\validate\WechatBindValidate;
...
class Account extends Api
{
    ...
    public $like_not_need_login = ['silentLogin', 'resetPwd','authLogin', 'register', 'login','mnplogin', 'codeurl', 'oalogin', 'oplogin','logout','smslogin', 'uinAppLogin', 'silentLogin'];
    ...
    public function register()
    {
        ...
        if($result !== false) {
            return JsonServer::success('注册成功');
        }
    ...
    {
        $params =(new UpdateUserValidate())->goCheck('updateUser');
        LoginLogic::updateUser($params,$this->user_id);
    ...
    {
        $params =(new ResetPwdValidate())->goCheck();
        $result = LoginLogic::resetPwd($params);
        if(true === $result) {
            return JsonServer::success('操作成功',[],1,1);
        }
        return JsonServer::error(LoginLogic::getError());
    ...
    {
        $params = (new WechatBindValidate())->goCheck();
