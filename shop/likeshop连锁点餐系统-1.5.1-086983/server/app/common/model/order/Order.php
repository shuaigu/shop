...
//        $region_desc = implode('', $region);
     ...
     * @return mixed|string
     * @author ljj
     * @date 2024/2/2 10:20 上午
     */
    public function getShopDeskAttr($value,$data)
    {
        $result = ShopDesk::where(['id'=>$data['shop_desk_id']])->value('number');
     ...
     * @return array
     * @author ljj
     * @date 2024/2/18 6:14 下午
     */
    public function getDeskInfoAttr($value,$data)
    {
        $result = ShopDesk::where(['id'=>$data['shop_desk_id']])->field('id,number')->findOrEmpty()->toArray();
        return $result;
    }
}