...
class ShopOrderValidate extends Validate
{
    ...
    public function checkConfirm($value,$rule,$data)
    {
        foreach ($value as $val) {
            ...
            if ($result['dining_type'] == OrderEnum::DINING_TYPE_TAKE_EAT && $result['order_status'] != OrderEnum::ORDER_STATUS_GOODS) {
                return '订单'.$result['id'].'无法确认取餐';
            }
            if ($result['dining_type'] == OrderEnum::DINING_TYPE_EAT_IN && $result['order_status'] < OrderEnum::ORDER_STATUS_DELIVERY) {
                return '订单'.$result['id'].'无法完成订单';
            }
        }
        return true;
    }
