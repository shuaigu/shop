<template>
	<view class="uni-load-more" @click="onClick">
		...
		<view
			...
			<view class="uni-load-more__img-icon"
				:style="{borderTopColor:color,borderTopWidth:iconSize/12}"></view>
			<view class="uni-load-more__img-icon"
				:style="{borderTopColor:color,borderTopWidth:iconSize/12}"></view>
			<view class="uni-load-more__img-icon"
				:style="{borderTopColor:color,borderTopWidth:iconSize/12}"></view>
		...
		<view v-else-if="!webviewHide && status === 'loading' && showIcon"
			...
			<image class="uni-load-more__img-icon" :src="imgBase64" mode="widthFix"></image>
		</view>
		<!-- #endif -->
...
<script>
	...
	setTimeout( ( ) => {
		...
		try {
			...
			// 降级处理，如果getDeviceInfo不可用，尝试使用getSystemInfoSync
			try {
...
<style lang="scss">
	...
	.uni-load-more__img--ios-H5 .uni-load-more__img-icon {
		position: absolute;
		width: 100%;
		height: 100%;
		left: 0;
