...
class service {
	...
	async paymentNotify(data = {}) {
		...
		const returnNotifySUCCESS = typeof uniPayInstance.returnNotifySuccess === "function" ? uniPayInstance.returnNotifySuccess : libs.common.returnNotifySUCCESS;
		...
		if (notifyType !== "payment") {
			// 非支付通知直接返回成功
			console.log(`---------!非支付通知!---------`);
			return returnNotifySUCCESS({ provider, provider_pay_type });
		}
		// 支付通知，验证签名
	...
	async createOrder(data = {}) {
		...
		// 拼接实际异步回调地址
		let currentNotifyPath = `${notifyPath}${provider}-${provider_pay_type}`;
		...
		try {
			...
			if (provider !== "appleiap") {
				...
				if (provider === "wxpay-virtual") {
					...
					getOrderInfoParam.bizType = biz_type;
				}
	...
	async refund(data = {}) {
		...
		if (payOrderInfo.provider === "huawei") {
			refundParams.notifyUrl = `${payOrderInfo.notify_url}${payOrderInfo.notify_path}`;
		}
	...
	async getRefund(data = {}) {
		...
		try {
			...
			console.log('refundQueryJson: ', refundQueryJson)
